...
Humans (OIDC) and agents (agent keys / OAuth tokens) are first-class principals evaluated by the same ACL engine.

## Roles

- **Org roles**: `owner`, `admin`, `editor`, `viewer` (plus `member` for non-operators).
