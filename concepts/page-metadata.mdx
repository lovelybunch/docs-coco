...
- **Queryable**: `eq`, `neq`, `exists`, `missing`, `gt/gte/lt/lte` (numbers and ISO-date strings), `contains` (array membership), and `in` / `not-in` (the value is any, or none, of a list of candidates). Filters are AND-ed together, with `orderBy` ranking.
...
<Card title="Next: Record types" icon="table-list" href="/concepts/records">
  Schemas over metadata, so a space of documents is also a queryable dataset.
</Card>
